...
for row in readerCITIES:
    ...
    ciudad = row['en_label']
    if ciudad not in listadeciudades:
        listadeciudades.append(ciudad)
...
for row in readerCOUNTRIES:
    ...
    if country not in listadecountries:
        listadecountries.append(country)        
#-------------------------------------------------------------        
...
mycsv = open('PREkatakanaPLACES_EASY.csv', 'a', encoding="utf8")
...
for row in readerCITIES:
    ...
    #DISTRACTORS: Other city names of similar length
    posiblesdistractores = [] #all possible distractors

    for ciudadposible in listadeciudades: #I'll check similarity
        if len(ciudadposible) == len(ciudad) or len(ciudadposible) == (len(ciudad) + 1) or len(ciudadposible) == (len(ciudad) - 1):
    ...
    if distractor2 != ciudad and distractor2 != distractor1:
        #Writing the csv
...
for row in readerCOUNTRIES:
    ...
    #DISTRACTORS: Other country names of similar length
    posiblesdistractores = [] #all possible distractors

    for countryposible in listadecountries: #I'll check similarity
        if len(countryposible) == len(country) or len(countryposible) == (len(country) + 1) or len(countryposible) == (len(country) - 1):
    ...
    if distractor2 != country and distractor2 != distractor1:
        #Writing the csv
...
filecondoblelinebreak = open('PREkatakanaPLACES_EASY.csv', encoding="utf8")
...
with io.open("katakanaPLACES_EASY.csv",'a+',encoding='utf8') as f:
